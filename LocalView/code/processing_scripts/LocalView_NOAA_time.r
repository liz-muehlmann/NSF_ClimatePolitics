...
load("./LocalView/data/modified/noaa_episodeLevel.rdata")
...
place_geo <- st_read("./GIS/original/2020_CensusSubNational/tlgdb_2020_a_us_substategeo.gdb", layer = "Incorporated_Place") %>% 
    select(GEOID, SHAPE) %>% 
    rename(place_fips = GEOID,
           place_shape = SHAPE) %>% 
    rbind(cdp_geo)
...
lv <- allData_transcriptLevel %>%
    ...
    select(-transcript_day, -transcript_month, -starts_with("fema_"))

noaa <- noaa_episodeLevel %>% 
    select(noaa_episodeID, stcounty_fips, noaa_year, noaa_episode_beginDate, noaa_nEventsInEpisode,
           noaa_nEventTypesInEpisode, noaa_nEpisodeCountyYear, noaa_nEventCountyYear,
           noaa_episode_injuriesDirect, noaa_episode_injuriesIndirect, noaa_episode_deathsDirect,
...
# save(lvNoaa, file = "./LocalView/data/modified/lvNoaa_all.rdata")
...
save(lvNoaa_transcriptLevel, file = "./LocalView/data/modified/lvNoaa_transcriptLevel.rdata")
