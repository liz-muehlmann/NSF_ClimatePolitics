...
nearestDecl_beforeMeeting <- lvFema %>%
  group_by(stcounty_fips, meeting_date) %>%
  filter(days_since_declaration < 0 & days_since_declaration == 
             min(days_since_declaration)) %>%
  select(
    state_name, county_name, stcounty_fips, ccBinary, n_ccMentions, 
    fema_decInCountyYear,fema_decTypeCountyYear, n_declarations_onCountyDate, 
    meeting_date, declaration_date, days_since_declaration
  )
