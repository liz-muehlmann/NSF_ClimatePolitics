### file description ###############################################################################
##                                                                                                ##
## This file handles the processing steps necessary for the FEMA disaster declarations            ##
##      Data included:                                                                            ##
##          FEMA (2010-2023)                                                                      ## 
...
fema <- read.csv("./LocalView/data/original/2023_FEMA.csv") %>% 
    ...
    mutate(fema_decInCountyYear = n(),
           fema_decTypeCountyYear = n_distinct(fema_declarationType)) %>% 
    ungroup() %>% 
    select(fema_id, fema_year, fema_month, fema_day, fema_declarationType, fema_incidentType, 
           state_fips, county_fips, stcounty_fips, fema_decInCountyYear, fema_decTypeCountyYear) %>% 
    excludeStates() 
...
fema_countyYear <- bind_rows(f_merged) %>% 
    ...
    select(stcounty_fips, fema_year, transcript_year, fema_decInCountyYear, fema_decTypeCountyYear, fema_binary)
