...
##      county-year level. Individual data processing files are linked below. Detailed            ##
...
load("./LocalView/data/modified/lv_countyYear_withNA.rdata")
...
for (y in unique(lv_countyYear_withNA$transcript_year)){
    ad <- lv_countyYear_withNA %>% 
...
allData_withNA <- left_join(lvacsru_algara, cvi, 
                    by = "stcounty_fips", 
                    relationship = "many-to-one") %>% 
                  left_join(fema_countyYear) %>% 
    relocate(census_division, census_region, transcript_year, has_transcript, .after = county_fips) %>% 
    relocate(ru_year, .after = edu_percentPop)

# save(allData_withNA, file = "./LocalView/data/modified/allData_withNA.rdata")
