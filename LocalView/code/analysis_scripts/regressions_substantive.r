...
model_controls <- lm(prop_ccMentions ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
...
model_divisionFE <- plm(prop_ccMentions ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                        edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi  + as.factor(fema_binary),
...
model_linear_time <- lm(prop_ccMentions ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                        edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi + as.factor(fema_binary) +
...
model_dummy_time <- lm(prop_ccMentions ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                       edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi  + as.factor(fema_binary) +
...
for(y in unique(allData_noNA$transcript_year)){
    ...
    if(y == 2010){
        ## 2010 has no transcripts in with prior year FEMA declaration
         ...
         m_sub <- lm(prop_ccMentions ~ DVP + perc_white + log(med_hhic) + edu_percentPop + 
...
model_controls <- lm(prop_scriptCCMention ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                         edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi + as.factor(fema_binary),
                     data = allData_withNA)
...
model_divisionFE <- plm(prop_scriptCCMention ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                            edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi  + as.factor(fema_binary),
                        data = allData_withNA, index = c("census_division"), model="within")
...
model_linear_time <- lm(prop_scriptCCMention ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                            edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi + as.factor(fema_binary) +
                            as.numeric(transcript_year), data = allData_withNA)
...
model_dummy_time <- lm(prop_scriptCCMention ~ DVP + log(total_pop) + log(med_hhic) + med_age + perc_white + 
                           edu_percentPop + as.factor(rural_urban_3pt) + overall_cvi  + as.factor(fema_binary) +
                           as.factor(transcript_year), data = allData_withNA)
...
for(y in unique(allData_noNA$transcript_year)){
    ...
    if(y == 2010){
        ...
        m_sub <- lm(prop_scriptCCMention ~ DVP + perc_white + log(med_hhic) + edu_percentPop + 
                        as.factor(rural_urban_3pt) + overall_cvi + log(total_pop) + fema_binary, 
                    data = df)
